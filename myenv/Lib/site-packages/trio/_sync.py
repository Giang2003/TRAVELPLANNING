...
from . import _core
from ._core import Abort, Bãi đậu xeLot, RaiseCancelT, enable_ki_protection
...
if TYPE_CHECKING:
    ...
    from ._core._parking_lot import Bãi đậu xeLotStatistics
...
class Event:
    ...
    or :class:`trio.lowlevel.Bãi đậu xeLot`.
...
class Semaphore(AsyncContextManagerMixin):
    ...
    def __init__(self, initial_value: int, *, max_value: int | None = None):
        ...
        self._lot = trio.lowlevel.Bãi đậu xeLot()
...
class _LockImpl(AsyncContextManagerMixin):
    _lot: Bãi đậu xeLot = attrs.field(factory=Bãi đậu xeLot, init=False)
