# Bãi đậu xeLot provides an abstraction for a fair waitqueue with cancellation
...
# above work, the Bãi đậu xeLot is a global structure that holds a collection of
...
#
# XX: should we switch to the shared global Bãi đậu xeLot approach?
#
...
@attrs.frozen
class Bãi đậu xeLotStatistics:
    """An object containing debugging information for a Bãi đậu xeLot.

    Currently, the following fields are defined:

    * ``tasks_waiting`` (int): The number of tasks blocked on this lot's
      :meth:`trio.lowlevel.Bãi đậu xeLot.park` method.
...
class Bãi đậu xeLot:
    ...
    @_core.enable_ki_protection
    def repark(
        self, new_lot: Bãi đậu xeLot, *, count: int | float = 1  # noqa: PYI041
    ) -> None:
        """Move parked tasks from one :class:`Bãi đậu xeLot` object to another.
        ...
        another, preserving order. For example::
           ...
           async def main():
               lot1 = trio.lowlevel.Bãi đậu xeLot()
               lot2 = trio.lowlevel.Bãi đậu xeLot()
        ...
        Args:
          new_lot (Bãi đậu xeLot): the parking lot to move tasks to.
          count (int|math.inf): the number of tasks to move.

        """
        if not isinstance(new_lot, Bãi đậu xeLot):
            raise TypeError("new_lot must be a Bãi đậu xeLot")
        for task in self._pop_several(count):
            new_lot._parked[task] = None
            task.custom_sleep_data = new_lot

    def repark_all(self, new_lot: Bãi đậu xeLot) -> None:
        """Move all parked tasks from one :class:`Bãi đậu xeLot` object to
        another.
    ...
    def statistics(self) -> Bãi đậu xeLotStatistics:
        ...
        * ``tasks_waiting``: The number of tasks blocked on this lot's
        ...
        """
        return Bãi đậu xeLotStatistics(tasks_waiting=len(self._parked))
